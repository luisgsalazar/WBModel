...
WBModel <- function( MaxIterations    = 10,
                     ...
                     SurvivalProbAdF  = 0.9,  #0.9986^365,                   # Yearly Survival Prob for adults Kueling et al. (2013)    
                     SurvivalProbAdM  = 0.9,  #0.9986^365,                   # Yearly Survival Prob for adults Kueling et al. (2013) 
                     SurvivalProbSAdF = 0.9,  #0.9988^365,                   # Yearly Survival Prob for sub-adults (we use adults values as they are closer to Lange et al. 2015) Kueling et al. (2013)
                     SurvivalProbSAdM = 0.9,  #0.9986^365,                   # Yearly Survival Prob for sub-adults(we use adults values as they are closer to Lange et al. 2015) Kueling et al. (2013)    
                     ...
                     runID            = paste0("WB_Model", "Dep64_0.6")
){
...
# Initialize Model Outcomes.  ---------------------------------------------
  ...
  DayOutCarcMat  <- matrix(0, ncol = MaxIterations, nrow = MaxDays)
  ...
  if(gTime %in% ((28*7 + ((Year - 1)*365)) + 0:6)){
    ...
    if(dim(GroupSplitNumSD)[1] > 0){
      ...
      if(gTime >= (25*7 + ((Year - 1)*365)) & gTime<=(30*7 + ((Year - 1)*365))){
        ...
        table(PopMatWB[ ,2],PopMatWB[ ,3],PopMatWB[ ,4] ,
              PopMatWB[ ,12])[,1,2,1]->tmp
...
# ASF Dynamics ------------------------------------------------------------
  ...
  ## Bot from their group and their neighbours. 
        ...
        # }
          ...
          #   
            ...
            #   MovedGroupsTod <- GroupsMoved[MovedGroups]
            #   if(length(MovedGroupsTod) > 0){
      ...
      DayOutPopMat[gTime, Iter] <- length(unique(PopMatWB[PopMatWB[,9] %in% c(0:2,4) ,2]))
      DayOutAniMat[gTime, Iter] <- nrow(PopMatWB[PopMatWB[,9] %in% c(0:2,4) ,])
      DayOutCarcMat[gTime, Iter] <- length(PopMatWB[PopMatWB[,9] == 3 ,8])
     ...
     #   Popcount <- PopMatWB[PopMatWB[ ,9] %in% c(0,1,2,4),] %>% group_by(Current_pixel) %>%
     #     dplyr::summarise(n())
     #   Popcount <- t(Popcount)
  ...
  NAME_CARC <- paste(runID, "FDayOutCarcMat.txt" , sep = "-")
  NAME_IC <- paste(runID, "FDayOutInfPerCell.txt" , sep = "-")
  ...
  write.table(DayOutCarcMat, NAME_CARC, sep = " ", col.names = T, row.names = T)
  write.table(InfectedPerCells, NAME_IC, sep = " ", col.names = FALSE, row.names = FALSE)
...
#Scenarios <- data.frame(Sc = c(1:42),
                        ...
                        #HuntPigglet   = c(rep('NA', 2), rep('Yes', 2), 'NA', rep('Yes', 24), rep('No', 12), 'No'),
...
# 
# 
foreach (i = c(1:4), .export = c('HC2','HC3'), .packages = c("dplyr","flux"))  %dopar% {
  path = paste("./Outputs/Scenario", i, sep = '_')
